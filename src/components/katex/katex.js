...
.katex-error {
  color: var(--katex-error-color, #cc0000);
  padding: 0.5em;
}
...
:host(:not([block])) el-scrollbar::part(wrap) {
  ...
  updated(changedProperties) {
    ...
    if (changedProperties.has('expression')) {
      this.dispatchEvent(new CustomEvent('katex-rendered', {
        detail: { expression: this.expression },
        bubbles: true,
        composed: true,
      }));
    }
...
customElements.define('el-katex', Katex);